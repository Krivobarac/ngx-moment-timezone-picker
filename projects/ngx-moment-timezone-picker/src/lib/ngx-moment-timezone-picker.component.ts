...
@Component({
  selector: 'ngx-moment-timezone-picker',
  template: `
  ...
  `,
  styles: ['./moment-timezone-picker.scss']
})

export class NgxMomentTimezonePickerComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges, ControlValueAccessor {
  ...
  writeValue(zone: string | TZone): void {
    if (zone) {
      ...
      if (typeof zone === 'string' && zone.length > 0) {
        _zone = this.timeZones.find(z => z.nameValue === zone) || null;
      } else if (typeof zone === 'object') {
        _zone = this.timeZones.find(z => z.nameValue === zone.nameValue) || null;
      }
